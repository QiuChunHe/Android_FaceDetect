package com.moli.faceDetect.faceserver;
...
import com.moli.faceDetect.model.FaceRegisterInfo;
...
public class FaceServer {
     ...
     * @param
     */
    public Bitmap getHeadImageBitmap(byte[] bgrData, int width, int height, FaceInfo faceInfo) throws IOException {
        synchronized (this) {
            if (bgrData == null || faceInfo == null) {
                return null;
            }
            //人脸检测
//        List<FaceInfo> faceInfoList = new ArrayList<>();
//        int code = faceEngine.detectFaces(bgrData, width, height, FaceEngine.CP_PAF_BGR24, faceInfoList);
//        if (code == ErrorInfo.MOK && faceInfoList.size() > 0) {
//            FaceFeature faceFeature = new FaceFeature();FaceFeature

            //特征提取
//            code = faceEngine.extractFaceFeature(bgrData, width, height, FaceEngine.CP_PAF_BGR24, faceInfoList.get(0), faceFeature);
            //保存注册结果（注册图、特征数据）
//            if (code == ErrorInfo.MOK) {
            //为了美观，扩大rect截取注册图
            Rect cropRect = getBestRect(width, height, faceInfo.getRect());
...
//            cropRect.bottom &= ~3;
            ...
            // 创建一个头像的Bitmap，存放旋转结果图
            Bitmap headBmp = getHeadImage(bgrData, width, height, 0, cropRect, ArcSoftImageFormat.BGR24);
            return headBmp;
        }
//            }
//        }else {
...
//        return null;
     ...
     * @return
     */
    public ImageResult getImageBitmap(byte[] nv21, int width, int height, FaceInfo faceInfo) {
        synchronized (this) {
            if (nv21 == null || faceInfo == null || width % 4 != 0 || nv21.length != width * height * 3 / 2) {
                Log.e(TAG, "registerNv21: invalid params");
...
//                    File file = new File(imgDir + File.separator + userName + IMG_SUFFIX);
                    ...
                    headBmp.compress(Bitmap.CompressFormat.PNG, 100, outputStream);
...
//                    }
//                    faceRegisterInfoList.add(new FaceRegisterInfo(faceFeature.getFeatureData(), userName));
                    return new ImageResult(headBmp, byteArr, true);
//                } catch (IOException e) {
//                    e.printStackTrace();
//                    return new ImageResult(null, false);
//                }
